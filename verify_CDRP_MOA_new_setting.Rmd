...
for (k in ks) {
  res <- cr.melt %>% 
    ...
    dplyr::select(-has.a.match) %>% 
    dplyr::arrange(MOA.x) 
  D <- rbind(D, data.frame(k = k, meas = NROW(res)/NROW(cr)))  
  #print(sprintf("ratio of compounds with a same MOA compound being in their %d-NN : %f", k, NROW(res)/NROW(cr)))
}

g <- ggplot(D, aes(x = k, y = meas)) + ggplot2::geom_line() + ggplot2::ylab("ratio of same MOA compound pairs")
