...
Pf.cmpd <- readRDS("../results/master/2017-04-20_3f29403c/Pf_annotated.rds")
  ...
  for (gene in Pf.gene$data$Treatment) {
    ...
    if (fsh$p.value < 0.05/NROW(Pf.gene$data$Treatment)) {
      ...
      if (verbose) {
        ...
        Pf.cmpd$data %>% dplyr::filter(CPD_NAME %in% reg) %>% dplyr::select(one_of(c("CPD_NAME", "MOA", "Target"))) %>% dplyr::inner_join(., u.res.lim, by = "CPD_NAME") %>% htmlTable::htmlTable() %>% cat
...
res <- sample.tot(random.shuffle = F, verbose = T, cor.thr = 0.35, how.many = 10)
