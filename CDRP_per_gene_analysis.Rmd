  ...
  for (gene in Pf.gene$data$Treatment) {
    ...
    if (fsh$p.value < 0.05/NROW(Pf.gene$data$Treatment)) {
      ...
      if (verbose) {
        ...
        Pf.cmpd$data %>% dplyr::filter(CPD_NAME %in% reg) %>% dplyr::select(one_of(c("CPD_NAME", "MOA", "Target"))) %>% dplyr::inner_join(., u.res.lim, by = "CPD_NAME") %>% arrange(Corr.) %>% htmlTable::htmlTable() %>% cat
    ...
    Pf.cmpd$data %>% dplyr::filter(CPD_NAME %in% reg) %>% dplyr::select(one_of(c("CPD_NAME", "MOA", "Target"))) %>% dplyr::inner_join(., u.res.lim, by = "CPD_NAME") %>% arrange(Corr.) %>% htmlTable::htmlTable() %>% cat(., file = "predictions.html", append = T)
...
res <- sample.tot(random.shuffle = F, verbose = T, cor.thr = 0.40, how.many = 20)
