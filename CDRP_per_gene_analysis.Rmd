...
if (use.snf) {
  ...
  df.graph <- SNFtool::SNF(afs, K = 10, t = 10)
  cr <- df.graph
...
sample.tot <- function(Pf.cmpd, Pf.gene, cr.melt, verbose = F, cor.thr = 0.5, how.many = 5) {
  ...
  for (gene in Pf.gene$data$Treatment) {
    ...
    u.res <- cr.melt %>% 
      ...
      dplyr::arrange(-value) %>%
      dplyr::mutate(value = round(value, 5)) %>% dplyr::filter((value) > cor.thr) 
