...
sample.tot <- function(Pf.cmpd, Pf.gene, random.shuffle.genes = F, random.shuffle.cmpds = F, verbose = F, cor.thr = 0.5, how.many = 5) {
  gn.list <- c()
  ...
  for (gene in Pf.gene$data$Treatment) {
    ...
    if (fsh$p.value < 0.05/length(Pf.gene$data$Treatment)) {
...
res <- sample.tot(Pf.cmpd = Pf.cmpd, Pf.gene = Pf.gene, random.shuffle.genes = F, random.shuffle.cmpds = F, verbose = T, cor.thr = cor.thr, how.many = no.top.matches)
...
nl <- foreach (i = 1:N) %dopar% {
  res <- sample.tot(Pf.cmpd = Pf.cmpd, Pf.gene = Pf.gene, random.shuffle.genes = T, random.shuffle.cmpds = F, verbose = F, cor.thr = cor.thr, how.many = no.top.matches)
  length(res$genes)
}
