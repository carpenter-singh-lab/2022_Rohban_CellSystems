...
#   cmpds.cid <- lapply(cmpds, cmpd.name.to.cid) %>% unlist
  ...
  annot.res <- annot.res %>% dplyr::filter(InteractionBrief %in% c("increases_expression", "decreases_expression", "increases_activity", "decreases_activity", "increases_phosphorylation", "decreases_phosphorylation"))
  gene.chem.conn <- cr * 0
...
d.app <- d.app %>% dplyr::filter(Compound.Specificity > 0.95)
