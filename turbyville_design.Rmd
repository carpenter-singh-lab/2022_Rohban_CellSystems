...
where in the approved list
Result has 0 rows if true.

```{r}
simple_correlations %>%
  anti_join(mta_compounds %>% select(pert_id))
```
...
Before plotting, first remove all the compounds that were selected based on a simple correlation to
HRAS_G12V, KRAS_G12V, KRAS_WT_1 (30 each) 
