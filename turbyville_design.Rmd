...
ggplot(differential_compounds_mut_to_wt, aes(score)) + 
  ...
  geom_rug(data = differential_compounds_mut_to_wt %>% filter(is_available),
           color = "red") + 
  geom_rug(data = differential_compounds_mut_to_wt %>% filter(is_firstbatch),
           color = "blue") + 
  ggtitle("Mutant to WT", subtitle = "Red rug indicates available compounds, blue indicates those already in current plate.")
...
compounds_to_order <- 
  bind_rows(
    first_batch_platemap %>% filter(!is.na(broad_sample)) %>% select(broad_sample) %>% mutate(tag = "first_batch"),
