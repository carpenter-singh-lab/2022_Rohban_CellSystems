...
x.collapsed <- x %>% dplyr::group_by(Metadata_pert_iname, Metadata_pert_idose, Metadata_moa, Metadata_broad_sample) %>% 
  dplyr::select(one_of(c(feats, "Metadata_pert_iname", "Metadata_pert_idose", "Metadata_moa", "Metadata_broad_sample"))) %>% dplyr::summarise_each(funs("mean")) %>% dplyr::ungroup() %>% dplyr::filter(!is.na(Metadata_pert_iname))

Pf.repurp <- list(data = x.collapsed, feat_cols = feats, factor_cols = c("Metadata_pert_iname", "Metadata_pert_idose", "Metadata_moa", "Metadata_broad_sample"))
...
k <- 120
pr <- prcomp(rbind(Pf.cdrp$data[,f], Pf.repurp$data[,f]), center = F, scale. = F, retx = T)
d1 <- cbind(pr$x[1:NROW(Pf.cdrp$data),1:k] %>% as.data.frame(), Pf.cdrp$data[,Pf.cdrp$factor_cols] %>% as.data.frame())
d2 <- cbind(pr$x[(NROW(Pf.cdrp$data)+1):NROW(pr$x),1:k] %>% as.data.frame(), Pf.repurp$data[,Pf.repurp$factor_cols] %>% as.data.frame())

Pf.cdrp$data <- d1
Pf.repurp$data <- d2

Pf.cdrp$feat_cols <- setdiff(colnames(d1), Pf.cdrp$factor_cols)
Pf.repurp$feat_cols <- setdiff(colnames(d2), Pf.repurp$factor_cols)
...
```{r, eval=just.common.cmpds}

cmpd1 <- Pf.cdrp$data$Metadata_broad_sample %>% str_sub(., 1, 13)
cmpd2 <- Pf.repurp$data$Metadata_broad_sample %>% str_sub(., 1, 13)

cmpd <- intersect(cmpd1, cmpd2)

Pf.cdrp$data %<>%
  dplyr::filter(str_sub(Metadata_broad_sample, 1, 13) %in% cmpd) #%>%
  #dplyr::mutate(MOA = str_sub(Metadata_broad_sample, 1, 13))

Pf.repurp$data %<>%
  dplyr::filter(str_sub(Metadata_broad_sample, 1, 13) %in% cmpd) #%>%
  #dplyr::mutate(Metadata_moa = str_sub(Metadata_broad_sample, 1, 13))
