...
moas.to.consider <- c("glucocorticoid receptor agonist", "PLK inhibitor", "MEK inhibitor", "HDAC inhibitor", 
                      "HSP inhibitor", "mTOR inhibitor", "CDK inhibitor", "ribonucleoside reductase inhibitor", 
                      "MDM inhibitor", "EGFR inhibitor", "PI3K inhibitor", "VEGFR inhibitor", "p38 MAPK inhibitor", 
                      "PKC activator", "DNA synthesis inhibitor", "JAK inhibitor", "AKT inhibitor", "DNA inhibitor",
                      "dopamine receptor antagonist", "cholesterol inhibitor", "selective estrogen receptor modulator",
                      "tricyclic antidepressant", "angiotensin converting enzyme inhibitor", "src inhibitor", 
                      "retinoid receptor agonist", "serotonin reuptake inhibitor", "JNK inhibitor", "FLT3 inhibitor",
                      "PPAR receptor agonist", "HCV inhibitor", "RAF inhibitor", "rho associated kinase inhibitor",
                      "growth factor receptor inhibitor", "Abl kinase inhibitor", "PDGFR tyrosine kinase receptor inhibitor", 
                      "protein kinase inhibitor", "NFkB pathway inhibitor", "XIAP inhibitor", "PKC inhibitor", "FGFR inhibitor", 
                      "PARP inhibitor", "TRPV agonist")
...
for (moa in moas.to.consider) {
  ...
  up.reg <- u.res %>% head(., 5) %>% dplyr::select(Gene.Overexpression) %>% as.matrix() %>% as.vector()
  down.reg <- u.res %>% tail(., 5) %>% dplyr::select(Gene.Overexpression) %>% as.matrix() %>% as.vector()
  ...
  print(sprintf("Following genes are relevant : %s", paste(reg[i], " ", collapse = " ")))
  v11 <- length(i)
  v12 <- length(reg) - v11
