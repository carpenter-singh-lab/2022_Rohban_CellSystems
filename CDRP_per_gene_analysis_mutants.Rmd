...
specificity <- function(vals) {
  vals <- vals %>% as.matrix() %>% as.vector()
  ord <- order(vals, decreasing = T)
  return((mean(vals[ord[1:3]]) - mean(vals[ord[4:8]]))/(mean(vals[ord[1:3]])))
}

make.match <- function(x) {
  y <- x %>% 
    arrange(-value) %>%
    slice(c(1, NROW(x))) %>%
    select(Var2, value) 
...
matches <- cr.melt %>% 
  group_by(Var1) %>% 
  do(data.frame(make.match(.))) %>%
  dplyr::rename(best.gene.match = Var2, corr = value)

cr.melt %>%
  group_by(Var1) %>%
  do(data.frame(spec. = specificity(.[,"value"]))) %>%
  arrange(-spec.) %>%
  dplyr::left_join(., Pf.cmpd$data[, c("CPD_NAME", "MOA")], by = c("Var1" = "CPD_NAME")) %>% 
  dplyr::left_join(., matches, by = "Var1") %>% 
  filter(abs(corr) > 0.4) %>% 
