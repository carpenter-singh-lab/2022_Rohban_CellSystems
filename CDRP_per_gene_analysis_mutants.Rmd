...
specificity <- function(vals, pos.matches) {
  vals <- vals %>% as.matrix() %>% as.vector()
  ord <- order(vals, decreasing = pos.matches)
  diff <- (mean(vals[ord[1:3]]) - mean(vals[ord[4:8]]))/(mean(vals[ord[1:3]]))
  return(diff)
}
...
matches.pos <- cr.melt %>% 
  ...
  dplyr::rename(best.gene.match = Var2, corr = value)
...
pos.t <- cr.melt %>%
  group_by(Var1) %>%
  do(data.frame(spec. = specificity(.[,"value"], pos.matches = T))) %>%
  arrange(-spec.) %>%
  dplyr::left_join(., Pf.cmpd$data[, c("CPD_NAME", "MOA")], by = c("Var1" = "CPD_NAME")) %>% 
...
neg.t <- cr.melt %>%
  group_by(Var1) %>%
  do(data.frame(spec. = specificity(.[,"value"], pos.matches = F))) %>%
  arrange(-spec.) %>%
  dplyr::left_join(., Pf.cmpd$data[, c("CPD_NAME", "MOA")], by = c("Var1" = "CPD_NAME")) %>% 
...
rbind(pos.t, neg.t) %>% arrange(-spec.) %>% htmlTable::htmlTable()
...
rbind(pos.t, neg.t) %>% 
  dplyr::group_by(best.gene.match) %>%
  dplyr::summarise(spec.avg = mean(spec.)) %>%
  dplyr::mutate(spec.avg = round(spec.avg, 2)) %>%
  arrange(-spec.avg) %>%
  htmlTable::htmlTable()
