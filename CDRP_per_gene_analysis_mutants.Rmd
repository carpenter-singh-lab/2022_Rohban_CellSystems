...
cr.melt <- melt(cr)
saveRDS(cr.melt, "cr_melt_cp_canon.rds")
...
library(cmapR)
ds <- cmapR::parse.gctx("../results/manual/cmap.compound.score/result_WTCS.LM.COMBINED_n328x22962.gctx")
...
cr.l1000 %>% reshape2::melt() %>% saveRDS(., "cr_melt_l1000.rds")

```

```{r, eval = F}

Pf.trt.strong.collapsed$data %>% select(Treatment, Pathway) %>% unique %>% mutate(Gene = lapply(str_split(Treatment, "_"), function(x) x[1]), Allele_Description = lapply(str_split(Treatment, "_"), function(x) str_sub(x[2], 1, 2))) %>% filter(Allele_Description == "WT") %>% select(Gene, Pathway) %>% unique  %>% mutate(Pathway = str_replace(Pathway, "Canonical ", "")) %>% group_by(Pathway) %>% summarise(`Gene members` = paste0(Gene, collapse = ", ")) %>% htmlTable::htmlTable()
