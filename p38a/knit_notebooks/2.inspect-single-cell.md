...
``` r
plot_paper_figure <- function(test_type, annotate_column, strong_effect_threshold = 8) {
  ...
  if (annotate_column == "strong_effect") {
    p <- p +
      ggrepel::geom_text_repel(
...
``` r
p <- plot_paper_figure("ks", "strong_effect")
...
``` r
p <- plot_paper_figure("t", "strong_effect")
