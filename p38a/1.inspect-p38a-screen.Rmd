...
p <- 
  screen_data %>%
  ggplot(aes(Broad, -log10(AdjP), shape = is_cp_negcon, color = is_activator)) + 
  geom_point() + 
...
p <- 
  screen_data %>%
  ggplot(aes(Broad, medTLR5, shape = is_cp_negcon, color = is_activator)) + 
  geom_point() + 
  coord_cartesian(clip = "off") +
  xlab("Cell Painting Correlation") +
