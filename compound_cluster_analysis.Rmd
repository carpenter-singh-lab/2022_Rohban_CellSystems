...
plot.within.MOA <- function(MOA.name, cor.thr = 0.95, plot.mat = F) {
  brds <- cdrp.profiles %>% 
...
moas.list <- moa.corr %>% mutate(is.consistent = p.value < 0.005) %>% filter(is.consistent) %>% select(MOA) %>% as.matrix() %>% as.vector() %>% unique
...
v[v < 0.10] <- 0
corrplot::corrplot(v, method = "color", is.corr = F)
