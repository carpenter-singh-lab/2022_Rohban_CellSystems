...
plot.within.MOA <- function(MOA.name, plot.mat = F) {
  brds <- cdrp.profiles %>% 
  ...
  x <- cr[brds, brds]
  diag(x) <- NA
  y <- cr[brds, other.brds]
  
  z1 <- apply(x, 1, function(x) mean(x, na.rm = T))
  z2 <- apply(y, 1, function(x) mean(x, na.rm = T))
  if (length(z1) < 3) {
    return(1)
  }
  return(t.test(z1, z2, alternative = "greater", paired = T)$p.value)
}
