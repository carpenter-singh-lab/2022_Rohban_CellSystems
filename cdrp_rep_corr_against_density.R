...
g <- ggplot(u.joined, aes(x = cell.count, y = cr)) + 
  ...
  theme_bw() +
  geom_text(aes(0.5, 0.75, label = paste(as.character(round(u.summ[which(u.summ$pos.cr & 
                                                                         u.summ$pos.cell.count) , "freq"] * 100,
                                                           0)), "%")), size = 6, color = "gray50") + 
  geom_text(aes(-1.5, -0.18, label = paste(as.character(round(u.summ[which(!u.summ$pos.cr & 
                                                                           !u.summ$pos.cell.count) , "freq"] * 100, 
                                                           0)), "%")), size = 6, color = "gray50") +
  geom_text(aes(0.5, -0.18, label = paste(as.character(round(u.summ[which(!u.summ$pos.cr & 
                                                                          u.summ$pos.cell.count) , "freq"] * 100, 
                                                           0)), "%")), size = 6, color = "gray50") +
  geom_text(aes(-1.5, 0.75, label = paste(as.character(round(u.summ[which(u.summ$pos.cr & 
                                                                          !u.summ$pos.cell.count) , "freq"] * 100, 
                                                             0)), "%")), size = 6, color = "gray50") 
