...
Pf.cmpd$data <- Pf.cmpd$data %>%
  arrange(abs(Metadata_pert_idose - 10)) %>%
...
cr.melt <- melt(cr)
d <- readRDS("../input/repurp/2016_04_01_a549_48hr_batch1_normalized.rds")

cr.melt <- cr.melt %>% 
  left_join(d %>% select(Metadata_pert_iname, Metadata_moa) %>% unique, by = c("Var1" = "Metadata_pert_iname")) 
