...
sample.tot <- function(random.shuffle = F, verbose = F, cor.thr = 0.5, how.many = 5) {
  ...
  for (gene in Pf.gene$data$Treatment) {
    ...
    if (fsh$p.value < 0.05/73) {
      gn.list <- c(gn.list, gene)
      if (verbose) {
        print(gene)
        print(reg)
        print(main.targ)
        print(moas %>% dplyr::filter(Name %in% reg) %>% dplyr::select(one_of(c("Name", "MOA", "Target"))))
        print(ppi %>% dplyr::filter(Protein.1 == gene | Protein.2 == gene) %>% unique %>% head(., 20))
        print("---------------------------------------------")
      }
    }
  }
