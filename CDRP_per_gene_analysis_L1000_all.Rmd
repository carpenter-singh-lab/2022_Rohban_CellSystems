...
if (LUAD) {
  ...
  strong.trt <- Pf.trt.collapsed$data$Treatment
  data.annot <- Pf_org$data %>% dplyr::filter(Treatment %in% strong.trt) %>% dplyr::select(one_of(c("Treatment", "Pathway", "Gene", "AlleleDesc"))) %>% 
...
Pf.cmpd <- readRDS("../results/master/2017-04-29_7f7e6acb/Pf_annotated.rds")
Pf.cmpd$data %<>% mutate(CPD_NAME = ifelse(CPD_NAME != "", CPD_NAME, Metadata_broad_sample))

source("rep.corr.func.R")
Pf.1 <- readRDS("../results/master/2017-04-30_f9e15843/Pf_bio_new_all.rds")
if (!just.bioactives) {
...
ds <- parse.gctx("../results/manual/cmap.compound.score/result_WTCS.LM.COMBINED_n328x22962.gctx")
...
u <- cmpd.gene.score.cmap
cr.l1000 <- u
rownames(cr.l1000) <- rownames(cmpd.gene.score.cmap)

cr.melt.l1000 <- cr.l1000 %>% 
  reshape2::melt(.)

cr.melt.l1000.ext <- cr.melt.l1000 %>% 
  dplyr::left_join(., Pf.cmpd$data[,c("Metadata_broad_sample","CPD_NAME")], 
                   by = c("Var1" = "Metadata_broad_sample")) %>%
  dplyr::select(-Var1) %>%
  dplyr::rename(Var1 = CPD_NAME) %>%
  dplyr::filter(Var1 %in% rownames(cr))

cr.melt <- cr.melt.l1000.ext
  ...
  for (gene in Pf.gene$data$Treatment) {
    ...
    if (!random.shuffle) {
      u.res <- cr.melt %>% 
        ...
        dplyr::arrange(-value) %>%
        dplyr::mutate(value = round(value, 2)) %>% dplyr::filter(value > cor.thr) 
