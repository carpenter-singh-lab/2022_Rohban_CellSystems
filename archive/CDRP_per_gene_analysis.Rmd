...
Pf.cmpd$data %<>% ungroup() %>% mutate(CPD_NAME = ifelse(CPD_NAME != "", CPD_NAME, Metadata_broad_sample))
...
y2 <- y %>% 
  dplyr::left_join(., unique(moa2[,c("CPD_NAME", "Name", "MOA", "Target", "Metadata_broad_sample")]))
...
Pf.cmpd$factor_cols <- setdiff(Pf.cmpd$factor_cols, c("CPD_NAME_san", "Phase"))
...
sample.tot <- function(Pf.cmpd, Pf.gene, cr.melt, verbose = F, cor.thr = 0.5, how.many = 5) {
  ...
  for (gene in Pf.gene$data$Treatment) {
    ...
    pr <- main.targ
    for (p in pr) {
      if ((ppi %>% filter(Protein.1 == p & Protein.2 == p) %>% NROW()) == 0)
        ppi <- rbind(ppi, data.frame(Protein.1 = p, Protein.2 = p, Method = "", Type = "", Evidence = ""))
    }
    
    pr <- rest.targ
    for (p in pr) {
      if ((ppi2 %>% filter(Protein.1 == p & Protein.2 == p) %>% NROW()) == 0)
        ppi2 <- rbind(ppi2, data.frame(Protein.1 = p, Protein.2 = p, Method = "", Type = "", Evidence = ""))
    }}
