...
f <- intersect(f1, f2)
#f <- f[which(!str_detect(f, "AGP") & !str_detect(f, "RNA") & !str_detect(f, "DNA"))]
...
cr.gene.mat <- Pf.gene.all$data %>% dplyr::filter(Metadata_x_mutation_status == "EMPTY") %>% dplyr::select(one_of(Pf.gene.all$feat_cols)) %>% cor
...
cr.melt <- cr.melt %>% dplyr::filter(!is.infinite(value))
...
thr <- d.app$corr %>% abs
thr <- thr[which(!is.infinite(thr))] %>% quantile(., 0.99, na.rm = T) %>% as.matrix() %>% as.vector()
